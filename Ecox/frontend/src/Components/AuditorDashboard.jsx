// src/Components/AuditorDashboard.js
      ...
      {contracts.length !== 0 && (
        <table className="min-w-full bg-white border border-gray-200 shadow-md rounded-lg overflow-hidden my-4">
          <thead className="bg-slate-800 text-gray-800 uppercase text-sm leading-normal">
            <tr>
              <th style={{ color: "white" }} className="py-3 px-6 text-left">
                Project / Institution
              </th>
              <th style={{ color: "white" }} className="py-3 px-6 text-left">
                Symbol
              ...
              <th style={{ color: "white" }} className="py-3 px-6 text-center">
                Verification
              </th>
            </tr>
          </thead>
          <tbody className="text-gray-600 text-sm font-light">
            {contracts.map((contract, index) => (
              <tr
                className="border-b border-gray-400 hover:bg-gray-400"
                key={index}
              >
                ...
                <td className="py-3 px-6 text-left whitespace-nowrap">
                  {parseFloat(contract.salePrice, 4) * 84}$
                </td>
                <td className="py-3 px-6 text-left whitespace-nowrap">
                ...
                <td className="py-3 px-6 text-center">
                  <button
                    className={`bg-${
                      contract.verified || clickedButtons[index]
                        ? "gray-400"
                        : "green-400"
                    } hover:bg-${
                      contract.verified || clickedButtons[index]
                        ? "gray-500"
                        : "green-500"
                    } text-black font-semibold py-2 px-4 rounded-full`}
